...
# KNN(K-Nearest Neighbors, 최근접 이웃 알고리즘)을 통한 주가 예측
def PredictStock(df, beginVal):
    ...
    # 오늘의 시가를 이용하여 예측한다.
    y_predict = reg.predict([[beginVal]])
    #print(y_predict)
    ...
    # 디버그 출력
    print(f"knn 예측 종가: {y_predict[0]}, 예측 트렌드: {prediction_trend}")
